...
export async function GET(
  ...
  try {
    assertNamespace(ns);
    const r = await findNamespacedResourceByKind("Connection");
    if (!r) {
      return NextResponse.json(
        { error: "CRD kind=Connection not found in cluster" },
        { status: 404 },
      );
    }
    ...
    const out = items
      .map((it) => {
        ...
        return {
          ...
          host: typeof spec.host === "string" ? spec.host : null,
          port: typeof spec.port === "number" ? spec.port : null,
          database: typeof spec.database === "string" ? spec.database : null,
          user: typeof spec.user === "string" ? spec.user : null,
          ssl: typeof spec.ssl === "boolean" ? spec.ssl : null,
        };
      })
